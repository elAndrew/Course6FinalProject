...
The 95% confidnce interval contains the value 0, therefore we cannot reject the null hypothesis.  The data does not show that tooth length is affected by supplement type.
...
```
A dose of 0.5 and a dose of 2 have the largest mean difference, so let's use these values for our hypothesis testing.  Once again, our null hypothesis will be that the mean of dose0.5 and the mean of dose2 are not differnt.
```{r}
